...
The queue implementations we have seen here are pretty fast if we treat them as ephemeral, but if we want persistent queues, the amortised analysis doesn't apply. It *is* possible to combine amortised analysis and persistent data structures, but it requires a slightly different way of thinking. The problem with amortised analysis is that we have to save up ahead of time to pay for later expensive operations. And with persistent data structures we risk spending the savings more than once. Instead, it is possible, using lazy evaluation, to leave the data structure in a state where it has unevaluated operations that must be paid for in some future operations---instead of savings we have a depth. Any future series of operations we do on a data structure in such a state will have to pay off the depth before we can see the effect of the operation we delayed. We return to an example of this in the final section of this chapter.
